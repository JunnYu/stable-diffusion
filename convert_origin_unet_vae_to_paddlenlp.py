...
def convert_unet(unet, out_path):
    need_transpose = ['time_embedding.linear_1.weight',
...
def convert_vae(vae, out_path):
    need_transpose = ['encoder.mid_block.attentions.0.query.weight',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dump_path", default="laion400M_pretrain", type=str, help="Path to the output model.")
    ...
    converted_unet_checkpoint = convert_ldm_unet_checkpoint(unet_checkpoint, unet_config)

    # Convert the VAE model.
    vae_config = create_vae_diffusers_config(original_config)
    converted_vae_checkpoint = convert_ldm_vae_checkpoint(vae_checkpoint, vae_config)

    convert_unet(converted_unet_checkpoint, os.path.join(args.dump_path, "unet", "model_state.pdparams"))
    convert_vae(converted_vae_checkpoint, os.path.join(args.dump_path, "vqvae", "model_state.pdparams"))
